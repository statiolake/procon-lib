...
public:
  ...
  double distance_from(coord const &from) const {
    return cross(from - start, dir).len();
  }
...
bool has_intersection(segment const &lhs, sphere const &rhs) {
  ...
  if (eqdbl(vec_normal.len(), 0)) {
    ...
    PD_LIB(std::cerr << "[has_intersection] len of vec_normal is larger "
...
coord intersection(segment const &lhs, segment const &rhs) {
  ...
  if (base->is_line() && target->is_line()) {
    ...
    double d2 =
        base->distance_from(target->start + target->dir * target->len());
    double t = d1 / (d1 + d2);
