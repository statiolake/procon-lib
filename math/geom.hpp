...
class segment {
  ...
  public:
    ...
    vec normal_from(coord const &from) const {
        ...
        ASSERT(eqdbl(dot(normal, dir), 0), "[normal_from] can't calculate normal!");
        return normal;
    }
};
...
bool has_intersection(segment const &lhs, sphere const &rhs) {
    ...
    PD_LIB(std::cerr << "[has_intersection] vec_normal: " << vec_normal << " (length: " << vec_normal.length() << ")" << std::endl);
    if (eqdbl(vec_normal.length(), 0)) {
        ...
        PD_LIB(std::cerr << "[has_intersection] center_start: " << center_start << ", center_end: " << center_end << ", radius: " << rhs.radius << std::endl);
...
coord intersection(segment const &lhs, segment const &rhs) {
    ...
    if (base->is_line() && target->is_line()) {
        ...
        double d2 = base->distance_from(target->start +
